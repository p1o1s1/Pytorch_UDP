...
options :class:`~torch.distributed.ProcessGroupNCCL.Options`).
  ...
  processGroupGloo
      .def_static(
          "create_device",
          [](const std::string& hostname, const std::string& interface, const std::string& transportName)
              -> std::shared_ptr<::gloo::transport::Device> {
            if (!hostname.empty()) {
              return ::c10d::ProcessGroupGloo::createDeviceForHostname(
                  hostname, transportName);
            }
            if (!interface.empty()) {
              return ::c10d::ProcessGroupGloo::createDeviceForInterface(
                  interface, transportName);
            }
  ...
  processGroupGloo
      ...
      .def(
          py::init([](const c10::intrusive_ptr<::c10d::Store>& store,
            ...
            std::cout << "stupid trans =" <<  transportName << std::endl;
            if (ifnameEnv && strlen(ifnameEnv) > 1) {
              for (const auto& iface : split(',', ifnameEnv)) {
                options->devices.push_back(
                    ::c10d::ProcessGroupGloo::createDeviceForInterface(iface, transportName));
              }
            } else {
              ...
              options->devices.push_back(
                  ::c10d::ProcessGroupGloo::createDefaultDevice(transportName));
            }
