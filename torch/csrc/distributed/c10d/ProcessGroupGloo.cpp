...
std::shared_ptr<::gloo::transport::Device> ProcessGroupGloo::
    createDeviceForInterface(const std::string& interface_name, const std::string& transportName) {
  return ::c10d::GlooDeviceFactory::makeDeviceForInterface(interface_name, transportName);
}

std::shared_ptr<::gloo::transport::Device> ProcessGroupGloo::
    createDeviceForHostname(const std::string& hostname, const std::string& transportName) {
  TORCH_CHECK(
      doesHostnameResolveToUsableAddress(hostname),
      "Cannot resolve ",
      hostname,
      " to a (local) address");
  return ::c10d::GlooDeviceFactory::makeDeviceForHostname(hostname, transportName);
}

#if defined(__linux__) || defined(_WIN32)
std::shared_ptr<::gloo::transport::Device> ProcessGroupGloo::
    createDefaultDevice(const std::string& transportName) {
  ...
  if (doesHostnameResolveToUsableAddress(hostname.data())) {
    return ::c10d::GlooDeviceFactory::makeDeviceForHostname(hostname.data(), transportName);
  }
  ...
  return createDeviceForHostname(kLoopbackAddress, transportName);
}
#endif
...
std::shared_ptr<::gloo::transport::Device> ProcessGroupGloo::
  ...
  if (doesHostnameResolveToUsableAddress(hostname.get())) {
    return ::c10d::GlooDeviceFactory::makeDeviceForHostname(hostname.get(), transportName);
  }
...
ProcessGroupGloo::ProcessGroupGloo(
  ...
  std::cout << "stupid devices.size() =" << options->devices.size() << std::endl;
