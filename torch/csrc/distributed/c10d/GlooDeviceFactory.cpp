...
#endif
#if !GLOO_HAVE_TRANSPORT_UDP
#error "Expected the udp transport to be available on Linux."
#endif
#endif
...
C10_REGISTER_CREATOR(GlooDeviceRegistry, UDP, makeUDPDevice);
#endif
...
makeGlooDevice(const std::string& interfaceName, const std::string& hostName, const std::string& transportName)
{
...
std::shared_ptr<::gloo::transport::Device> GlooDeviceFactory::
  ...
  std::cout << "transportName1 = " << transportName << std::endl;
  if (!device) {
...
std::shared_ptr<::gloo::transport::Device> GlooDeviceFactory::
    makeDeviceForHostname(const std::string& hostname, const std::string& transportName) {
  auto device = makeGlooDevice("", hostname, transportName);
  std::cout << "transportName2 = " << transportName << std::endl;
  if (!device) {
